    ...
    <div class="container" >
        <div class="card" >
            <div class="card-header" >
                <a href="{{ URL::to('/') }}" >Categories</a> > <a href="{{ route('viewcategory', $congress->category_id) }}" >{{ $congress->category->name }}</a> > {{ $congress->title }}
            </div>
            <div class="card-body" >

                <div class="row" >
                    <div class="col-xl-12" style="margin-bottom: 20px" >
                        <a href="{{ route('viewcongress', $congress->id) }}" class="btn btn-primary" >All</a>
                        @foreach($years as $y)
                            <a class="btn btn-primary" href="{{ route('viewcongress', ['congress'=>$congress->id, 'year'=>$y->year]) }}" >{{ $y->year }}</a>
                        @endforeach
                    </div>
                    @php
